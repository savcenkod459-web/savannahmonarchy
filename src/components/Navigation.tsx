...
const Navigation = () => {
  ...
  const navItems = [
    { name: "Главная", path: "/" },
    { name: "О кошках", path: "/about", hasSubmenu: true },
    { name: "Каталог", path: "/catalog" },
    { name: "Заводчики", path: "/breeders" },
    { name: "Инструкция", path: "/guide" },
  ...
  const aboutSections = [
    { name: "О наших роскошных кошках", hash: "#luxury-cats", icon: Crown },
    { name: "Наши избранные породы", hash: "#featured-breeds", icon: Award },
    { name: "Редкость и Уникальность", hash: "#rarity", icon: Diamond },
    { name: "Цена", hash: "#pricing", icon: DollarSign },
    { name: "Важная информация", hash: "#important-info", icon: AlertCircle },
    { name: "Взрослые коты", hash: "#adult-cats", icon: Cat },
    { name: "Котята", hash: "#kittens", icon: Baby },
  ];
      ...
      <div className="container mx-auto px-6 py-5">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-3 group micro-interaction">
            ...
            <h1 className="text-2xl md:text-3xl font-display font-black text-luxury-gradient group-hover:scale-105 transition-transform luxury-text-shadow">
              LuxuryCats
            </h1>
          </Link>
          ...
          <div className="hidden lg:flex items-center space-x-10">
            {navItems.map((item) => {
                ...
                return (
                  <Popover key={item.path} open={aboutPopoverOpen} onOpenChange={setAboutPopoverOpen}>
                    <PopoverTrigger asChild>
                      <button
                        className={`text-sm font-semibold transition-all duration-300 relative group micro-interaction flex items-center gap-1 ${
                          isActive(item.path)
                            ? "text-primary luxury-text-shadow"
                            : "text-foreground/70 hover:text-primary"
                        }`}
                      >
                        {item.name}
                        <ChevronDown className="w-4 h-4" />
                        <span className={`absolute -bottom-1 left-0 h-0.5 bg-gradient-to-r from-primary to-accent transition-all duration-300 rounded-full ${
                          isActive(item.path) ? "w-full shadow-glow" : "w-0 group-hover:w-full"
                        }`} />
                      </button>
                    </PopoverTrigger>
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`text-sm font-semibold transition-all duration-300 relative group micro-interaction ${
                    isActive(item.path)
                      ? "text-primary luxury-text-shadow"
                      : "text-foreground/70 hover:text-primary"
                  }`}
            ...
            {isAdmin && (
              ...
              >
                ⚙️ Админ
                <span className={`absolute -bottom-1 left-0 h-0.5 bg-gradient-to-r from-primary to-accent transition-all duration-300 rounded-full ${
                  isActive("/admin/cats") ? "w-full shadow-glow" : "w-0 group-hover:w-full"
                }`} />
              </Link>
            )}
          ...
          <Button
            variant="ghost"
            size="icon"
            className="lg:hidden micro-interaction hover:shadow-gold"
        ...
        {/* Mobile Navigation */}
        {isOpen && (
          <div className="lg:hidden mt-4 animate-fade-in glass-card rounded-2xl">
            <ScrollArea className="h-[calc(100vh-140px)] px-4 py-4">
              <div className="space-y-2 pb-4">
                    ...
                    return (
                      <div key={item.path} className="space-y-1">
                        <Link
                          ...
                          className={`block py-3 px-4 rounded-xl transition-all duration-300 micro-interaction ${
                            isActive(item.path)
                              ? "bg-gradient-to-r from-primary to-accent text-primary-foreground shadow-gold font-semibold"
                ...
                {user ? (
                  <Button onClick={handleLogout} variant="ghost" className="w-full justify-start micro-interaction">
                    <LogOut className="h-4 w-4 mr-2" />
                    Выйти
                  </Button>
                ) : (
