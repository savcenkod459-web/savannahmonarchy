  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("cats").select("id, name, breed").eq("id", catId).single();
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("cat_pedigrees").select("*").eq("cat_id", catId).order("parent_type", {
        ascending: true
      });
      if (error) throw error;
  ...
  return <div className="min-h-screen">
      ...
      <main className="pt-24 pb-20">
        <ScrollAnimationWrapper animation="fade" delay={100}>
          ...
          {!pedigree || pedigree.length === 0 ? <div className="text-center py-20">
              <p className="text-xl text-muted-foreground">
                Информация о родословной пока не добавлена
              </p>
            </div> : <div className="grid md:grid-cols-2 gap-8">
              {/* Отец */}
              {fathers.length > 0 && <div>
                  <h2 className="text-3xl font-display font-bold text-primary mb-6 text-center">
                    Самец
                  </h2>
                  <div className="space-y-6">
                    {fathers.map(parent => <Card key={parent.id} className="group animate-scale-in overflow-hidden glass-card border-primary/20 shadow-soft hover:shadow-[0_0_60px_rgba(217,179,112,0.6)] transition-all duration-500 hover:scale-[1.02] hover:-translate-y-2 relative">
                        ...
                        <CardContent className="p-0 relative">
                          {parent.parent_images && parent.parent_images.length > 0 && <div className="aspect-[3/4] overflow-hidden relative cursor-pointer" onClick={() => openGallery(parent.parent_images, 0)}>
                              ...
                              <img src={parent.parent_images[0]} alt={parent.parent_name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                            </div>}
                          <div className="p-6 space-y-4 bg-gradient-to-b from-background/80 to-background">
                            ...
                            {parent.parent_description && <p className="text-foreground/80 text-sm leading-relaxed">
                                {parent.parent_description}
                              </p>}
                            {parent.parent_images && parent.parent_images.length > 1 && <div className="grid grid-cols-3 gap-2 pt-4">
                                {parent.parent_images.slice(1, 4).map((img, idx) => <div key={idx} className="relative group/img overflow-hidden rounded-lg cursor-pointer" onClick={() => openGallery(parent.parent_images, idx + 1)}>
                                    <img src={img} alt={`${parent.parent_name} ${idx + 2}`} className="aspect-square object-cover transition-transform duration-500 group-hover/img:scale-110" />
              ...
              {/* Мать */}
              {mothers.length > 0 && <div>
                  <h2 className="text-3xl font-display font-bold text-primary mb-6 text-center">
                    Самка
                  </h2>
                  <div className="space-y-6">
                    {mothers.map(parent => <Card key={parent.id} className="group animate-scale-in overflow-hidden glass-card border-primary/20 shadow-soft hover:shadow-[0_0_60px_rgba(217,179,112,0.6)] transition-all duration-500 hover:scale-[1.02] hover:-translate-y-2 relative">
                        <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-accent/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                        <div className="absolute inset-0 bg-gradient-to-br from-primary/20 via-accent/20 to-primary/20 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-md -z-10" />
                        <CardContent className="p-0 relative">
                          {parent.parent_images && parent.parent_images.length > 0 && <div className="aspect-[3/4] overflow-hidden relative cursor-pointer" onClick={() => openGallery(parent.parent_images, 0)}>
        ...
        <DialogContent className="max-w-[95vw] max-h-[95vh] p-0 bg-black/95 border-primary/20">
          ...
          <div className="relative w-full h-[95vh] flex items-center justify-center">
            ...
            {galleryImages.length > 1 && <>
                <Button variant="ghost" size="icon" className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white" onClick={prevImage}>
                  <ChevronLeft className="w-8 h-8" />
                </Button>
                <Button variant="ghost" size="icon" className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white" onClick={nextImage}>
